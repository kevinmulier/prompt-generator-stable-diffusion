...
class PromptGenerator {
	constructor() {
		...
		this.generatorOptionsButton = document.querySelector(
	...
	generatePrompt() {
		...
		const selectedPortraitShot = document.querySelector(
			"#portraitShotSelect"
		).value;

		// Define options for portrait shot types
		const portraitShotOptions = [
			"Full-Length Shot",
			"American Shot",
			"Medium Shot",
			"Close-Up Shot",
		...
		const randomizedPortraitShot =
			portraitShotOptions[
				Math.floor(Math.random() * portraitShotOptions.length)
			];
		...
		const selectedLandscapesShot = document.querySelector(
			"#landscapesShotSelect"
		).value;

		// Define options for landscape shot types
		const landscapesShotOptions = [
			"Long Shot",
			"Medium Shot",
			"Close-Up Shot",
			"Extreme Close-Up Shot",
		...
		const randomizedLandscapesShot =
			landscapesShotOptions[
				Math.floor(Math.random() * landscapesShotOptions.length)
			];
		...
		const isAdjectivesActive =
			document.querySelector("#adjectivesActive").checked;
		...
		for (let i = 0; i < numArtists; i++) {
			if (i < this.currentArtists.length) {
				artistsPrompt += `${this.randomElement(
					this.currentArtists.filter(
						(artist) => !artistsPrompt.includes(artist)
					)
				)}, `;
			}
		}
		...
		for (let i = 0; i < numStyles; i++) {
			if (i < this.currentStyles.length) {
				stylesPrompt += `${this.randomElement(
					this.currentStyles.filter((style) => !stylesPrompt.includes(style))
				)}, `;
			}
		}
		...
		if (isAdjectivesActive) {
			...
			if (this.currentAdjectives.length > 1) {
				prompt += `, ${this.randomElement(
					this.currentAdjectives.filter((adjective) => adjective !== adjective1)
				)}`;
			}
		}
	...
	async copyPromptsToClipboard(promptsArray) {
		...
		copyMessage.className =
			"absolute dark:text-fublu-50 text-fublu-500 font-bold -left-14 top-2.5";
...
document
	.querySelector("#promptsCopyButton")
	.addEventListener("click", async (event) => {
		await promptGenerator.copyPromptsToClipboard(
			promptGenerator.currentPrompts
		);
	});

// Event Listener To Toggle Dark Mode
document
	.querySelectorAll(".toggleThemeBtn")
	.forEach((btn) => btn.addEventListener("click", toggleTheme));

// Event listener to generate prompts when pressing generate
document
	.querySelector("#generatePromptsButton")
	.addEventListener("click", (event) => {
		...
		if (!isNaN(promptsNumber) && promptsNumber > 0 && promptsNumber <= 10000) {
			...
			alert(
				"Please enter a number of prompts to generate between 1 and 10000."
			);
		}
	});
...
function showGeneratorOptions() {
	if (promptGenerator.shownOptions == 0) {
		promptGenerator.generatorOptionsButton.textContent =
			promptGenerator.generatorOptionsButton.textContent.replace(
				"Show",
				"Hide"
			);
		...
		promptGenerator.generatorOptionsButton.textContent =
			promptGenerator.generatorOptionsButton.innerHTML.replace("Hide", "Show");
		promptGenerator.shownOptions--;
	}
...
document
	.querySelector("#generatorOptionsButton")
	.addEventListener("click", showGeneratorOptions);

// add an event listener to reset generators options
document
	.querySelector("#resetInputsButton")
	.addEventListener("click", promptGenerator.resetUserInputs);
